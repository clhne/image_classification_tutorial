...
def print_activations(t):
    print(t.op.name,'',t.get_shape().as_list)   #get_shape获取一个TensorShape对象，as_list方法返回每一个维度数
...
def model():
    ...
    with tf.name_scope('conv1') as scope:          
        ...
        conv1=tf.nn.relu(bias,name=scope)    #这里返回的是一个tensor（一个张量类）
    ...
    #local Response Normalization(lrn1)
